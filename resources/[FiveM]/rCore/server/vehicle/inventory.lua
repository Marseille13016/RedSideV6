...
AddEventHandler('playerDropped', function (reason)
    saveAllVehicleInventory()
    print('player dropped save all vehicle inventory in cache')
end)
...
function saveAllVehicleInventory()
    for k,v in pairs(VehicleInventoryCache) do
        if v.owned == false then
            ...
            MySQL.Async.execute('UPDATE `veh_inventory` SET plate = @plate, money = @money, inventory = @inventory WHERE plate = @plate', {
                ["@plate"] = v.plate,
                ["@inventory"] = json.encode(v.inventory),
                ["@money"] = json.encode(v.money),
            }, function(rowsChanged) end)
            print("^2Saving ^7["..v.plate.."] to BDD.") -- debug
            local entity = NetworkGetEntityFromNetworkId(v.NetID)--bug potentiel ici ?
            if not DoesEntityExist(entity) then
                print("^1Removing ^7["..v.plate.."] from vehicle cache.") -- debug
                VehicleInventoryCache[v.plate] = nil
            end
...
AddEventHandler("core:GetVehicleInventory", function(token, plate, net, _owned)
    ...
    if info[1] ~= nil then
        ...
        if _owned then
            Citizen.CreateThread(function()
                MySQL.Async.execute('INSERT INTO `veh_inventory` VALUES (@plate, @inventory, @money)', {
                    ["@plate"] = plate,
