            ...
            while opened do
                ...
                RageUI.IsVisible(RMenu:Get('core', 'weed_trait'), false, false, false, function()
                    if not Trating then
                        ...
                        RageUI.Button("Préparer: ~b~"..self.w.prep1label, "6 Weed non traité requis", pInventory[self.w.item2Label] ~= nil, function(_,_,s)
                            if s then 
                                if pInventory[self.w.item2Label].count >= 6 then
                                    StartTrait(self.w.prep1item)
                                    Trating = true
                                    TriggerServerEvent(events.remove, token, GetFirstLabelFromItem(self.w.item2), 6)
                                end
                            end
                        end)
                        RageUI.Button("Préparer: ~b~"..self.w.prep2label, "5 Weed non traité requis", pInventory[self.w.item2Label] ~= nil, function(_,_,s)
                            if s then 
